// File: src/components/ide/dialogs/NewFileDialog.tsx
...
export const NewFileDialog: React.FC = () => {
  ...
  const isOpen = activeDialog === "newFile";
  ...
  const handleSubmit = (e?: React.FormEvent) => {
    ...
    createFile(trimmed, dialogTargetNodeId);
    toast({
      title: "File Created",
      description: `Successfully created "${trimmed}".`,
    });
    closeDialog();
  };
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && closeDialog()}>
      <DialogContent className="sm:max-w-[425px] dark:bg-slate-950 bg-white border border-slate-200 dark:border-slate-800 text-slate-900 dark:text-slate-100 font-sans">
        <DialogHeader>
          <DialogTitle className="text-sm font-bold tracking-tight uppercase font-mono">Create New File</DialogTitle>
        </DialogHeader>
